    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-1 bg-white border border-slate-200/80 rounded-2xl p-4.5 shadow-sm flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-row overflow-x-auto lg:flex-col gap-1.5 font-sans select-none scrollbar-thin pb-1 lg:pb-0">
              {categoriesList.map((cat) => {
                ...
                const isUserAdded = userCategories.includes(cat);

                return (
                  <div key={cat} className="relative group shrink-0 lg:shrink">
                    <button
                      onClick={() => setSelectedCategory(cat)}
                      className={`w-full flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer border ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <IconComponent className={`h-4 w-4 ${isSelected ? "text-emerald-400" : "text-slate-400 group-hover:text-slate-500"}`} />
                        <span>{cat}</span>
                      </div>

                      {count > 0 && cat !== "All" && (
                        <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-full leading-none transition-all ${
                          isSelected 
                    ...
                    {isUserAdded && (
                      <button
                        ...
                        className="absolute -top-1 -right-1 z-10 p-0.5 bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-full shadow-xs hover:scale-110 transition duration-150 cursor-pointer"
          ...
          {/* Workflow Sections Filter Block */}
          <div className="border-t border-slate-100 pt-4 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <span className="text-slate-800 font-bold text-sm font-sans flex items-center gap-1.5">
              ...
              {selectedSection !== "All" && (
                <button
                  ...
                  className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold px-2 py-0.5 rounded-full transition cursor-pointer"
                >
                  Clear Filter
                </button>
              )}
            ...
            <div className="flex flex-row overflow-x-auto lg:flex-col gap-1.5 font-sans select-none scrollbar-thin pb-1 lg:pb-0">
              <button
                ...
                className={`flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer border shrink-0 lg:shrink ${
                  selectedSection === "All"
                    ? "bg-emerald-950 text-white border-emerald-950 shadow-md shadow-emerald-950/10 font-bold"
                    : "bg-transparent text-slate-600 border-transparent hover:bg-slate-50 hover:text-slate-900 hover:border-slate-100"
                }`}
              >
                <div className="flex items-center gap-2">
                  <Grid className="h-3.5 w-3.5 text-slate-400 group-hover:text-amber-500" />
              ...
              {sectionsList.map((sec) => {
                ...
                return (
                  <div key={sec} className="relative group shrink-0 lg:shrink">
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-2 truncate">
                        <Tag className={`h-3.5 w-3.5 shrink-0 ${isSelected ? "text-emerald-400" : "text-slate-400"}`} />
                    ...
                    <button
                      ...
                      className="absolute -top-1 -right-1 z-10 p-0.5 bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-full shadow-xs hover:scale-110 transition duration-150 cursor-pointer animate-none"
            ...
            {/* Integrated Inline Section Creator Inside Side Menu */}
            <div className="pt-2">
              {isCreatingSection ? (
                <form
                  onSubmit={(e) => {
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 w-full py-2 rounded-xl text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200/40 border-dashed transition cursor-pointer"
                >
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 w-full">
          {/* Section: Independent Master Catalog Item Mapping (saved independently) */}
          {selectedCategory !== "All" && (
            <div className="bg-slate-50 border border-slate-200/95 rounded-2xl p-5 shadow-xs">
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-1.5 text-slate-800 font-bold text-xs font-sans">
                    <PackagePlus className="h-4 w-4 text-emerald-600" />
                    <span className="text-slate-800 font-black">Independent Master Category Mapping</span>
                  </div>
                  <h4 className="text-slate-800 text-sm font-bold font-sans">
                    Map items from Master Catalog into "{selectedCategory}"
                  </h4>
                  <p className="text-slate-400 text-[11px] font-sans">
                    Custom folder assignments are saved independently in your browser and do not modify the centralized databases.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  <div className="relative shrink-0">
                    <input
                      ...
                      placeholder="Type name / code to lookup..."
                      className="w-56 px-3 py-2 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-slate-800 font-sans"
                    />
                  ...
                  <select
                    ...
                    className="w-64 px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none text-slate-800 font-sans cursor-pointer"
                  >
                    <option value="">-- Choose Master Item ({searchedMasterItems.length} found) --</option>
                    {searchedMasterItems.map(item => (
                  ...
                  <button
                    ...
                    onClick={() => {
                      const item = items.find(it => it.Item_ID === selectedMasterItemId);
                      if (item) {
                        handleItemCategoryOverride(item.Item_ID, selectedCategory);
                        setSelectedMasterItemId("");
                        setMasterItemSearch("");
                      }
                    }}
                    className={`px-4.5 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1.5 ${
                  ...
                  >
                    ...
                    <span>Map to {selectedCategory}</span>
                  </button>
                </div>
              </div>
            </div>
